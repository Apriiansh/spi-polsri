...
class AddUserIdToKegiatan extends Migration
{
    public function up()
    {
        if (!$this->db->fieldExists('user_id', 'kegiatan')) {
    ...
    public function down()
    {
        ...
        $foreignKeys = $this->db->getForeignKeyData('kegiatan');
        foreach ($foreignKeys as $foreignKey) {
            if ($foreignKey->constraint_name === 'kegiatan_user_id_foreign') {
                $this->forge->dropForeignKey('kegiatan', 'kegiatan_user_id_foreign');
                break;
            }
        }
